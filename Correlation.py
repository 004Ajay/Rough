...
sig_x = math.sqrt(((n_div)*Xi_sq_sum) - x_mean)
sig_y = math.sqrt(((n_div)*Yi_sq_sum) - y_mean)
r = round(Pxy / (sig_x * sig_y), 4)
...
print(f"\nPxy: {round(Pxy, 4)}\t|\tσx: {round(sig_x, 4)}\t|\tσy: {round (sig_y, 4)}\t|\tr: {r}\n\n{'-'*83}")
